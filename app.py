...
def receive_text():
    ...
    if sender_history["previous"] == "syolo":
        ...
        cuisines = find_cuisines(location)
        # Take a sample of these choices
        cuisines = random.sample(cuisines, CUISINE_SAMPLE_SIZE)

        # Download images for a single cuisine, so the user is not blocked,
        # waiting for all the images of all the cuisines to be downloaded.
        # We can get the images for the other cuisines using multithreading.

        # Also, give preference to a first_cuisine if we already have images
        # the cuisine.
        # Note, this does introduce somewhat of a bias towards cuisines that
        # have been selected previously in previous sessions.

        # Keep a deep copy of the list of cuisines, to account for the pop.
        ...
        while not found_cuisine_in_redis and idx < len(cuisines):
            ...
            if cuisine in redis:
                first_cuisine = cuisines.pop(idx)
                found_cuisine_in_redis = True
            idx += 1
        if not found_cuisine_in_redis:
            first_cuisine = cuisines.pop()

        # In the user's next request, first_cuisine will be the user's
        # previously sent cuisine.
        sender_history["previousCuisine"] = first_cuisine

        # Find images for this cuisine if we don't have any yet.
        if not redis.hexists("cuisines", first_cuisine):
            add_cuisine_images_to_redis(first_cuisine, redis)

        # Spin a new thread that finds images for each
        # cuisine for which don't have images.
        for cuisine in cuisines:
            if not redis.hexists("cuisines", cuisine):
                redis.hset("cuisines", cuisine, True)
                thread = Thread(target=add_cuisine_images_to_redis,
                                args=(cuisine, redis))
                thread.daemon = True
                thread.start()
        ...
        # The user is creating a party instead.
            ...
            party_name = generate_party_name(ADJECTIVES, FOOD_NAMES, redis)
...
def generate_party_name(adjectives, food_names, redis):
    # Pull out an adjective and a food name
    adjective = random.sample(adjectives, 1)[0].strip()
    food_name = random.sample(food_names, 1)[0].strip()
    ...
    while redis.sismember("parties", party_name) and retries < 5:
        party_name = generate_party_name(ADJECTIVES, FOOD_NAMES)
        retries += 1
...
if __name__ == "__main__":
    try:
        ...
        with open("adjectives.txt") as ADJECTIVES:
            ADJECTIVES = set(ADJECTIVES.readlines())
        with open("food_names.txt") as FOOD_NAMES:
            FOOD_NAMES = set(FOOD_NAMES.readlines())
